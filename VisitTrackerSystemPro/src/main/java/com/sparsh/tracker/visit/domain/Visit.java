...
@NamedQueries({
	@NamedQuery(
			name = "findAllVisits",
			query = "FROM Visit visit WHERE visit.outTime IS NULL"),
	@NamedQuery(
			name = "getVisitsForDate",
			query = "FROM Visit visit WHERE visit.scheduledOn = :scheduledDate and visit.outTime IS NULL"),
	@NamedQuery(
			name = "getVisitsForEmployeeAndDate",
			query = "FROM Visit visit WHERE " +
					"visit.employee.employeeNumber = :employeeNumber " +
					"AND visit.scheduledOn = :scheduledDate " +
					"AND visit.outTime IS NULL " +
					"AND visit.isConfirmed = false "+
					"AND visit.isCanceled = false"),
	@NamedQuery(
			name = "getNonConfirmedVisitsForEmployee",
			query = "FROM Visit visit WHERE " +
					"visit.employee.employeeNumber = :employeeNumber " +
					"AND visit.inTime IS NOT NULL " +
					"AND visit.outTime IS NULL " +
					"AND visit.isConfirmed = false "+
					"AND visit.isCanceled = false"),
	@NamedQuery(
			name = "getCancellableVisitsForEmployee",
			query = "FROM Visit visit WHERE " +
					"visit.employee.employeeNumber = :employeeNumber " +
					"AND visit.inTime IS NULL " +
					"AND visit.outTime IS NULL " +
					"AND visit.isConfirmed = false " +
					"AND visit.isCanceled = false"),
	@NamedQuery(
			name = "getVisitorNames",
			query = "SELECT DISTINCT visit.visitorName FROM Visit visit WHERE visit.visitorName like :visitorName"),
	@NamedQuery(
			name = "findVisitByInTime",
			query = "FROM Visit visit WHERE visit.inTime = :inTime and visit.isCanceled = :isCanceled"),
	@NamedQuery(
			name = "getVisitsAfterDate",
			query = "FROM Visit visit WHERE visit.inTime >= :fromDate ORDER BY visit.inTime"),
	@NamedQuery(
			name = "getVistsForDateRange",
